...
subtitle: "Proposal for A Multicenter Retrospective Observational Cohort Study using the Multicenter Perioperative Outcomes Group (MPOG) Database"
...
thanks: |-
  This is a revised proposal prepared for review by the Perioperative Clinical Research Committee (PCRC) following initial recommendation for electronic revisions.
...
renoir <- MetBrewer::met.brewer("Renoir")
...
This is a retrospective cohort study with IRB approval from the Duke University Health System (Pro00112464). All future documentation will adhere to reporting guidelines as outlined by the Strengthening the Reporting of Observational Studies in Epidemiology (STROBE) statement[@RN1201], the REporting of studies Conducted using Observational Routinely-collected Data (RECORD) statement[@RN1237], as well as guidelines for the conduct and reporting of Bayesian analyses.[@RN1200; @RN1223] A preliminary RECORD checklist has been submitted in tandem with this proposal. In an effort to promote open science and reproducibility, the majority of the data processing and analytic scripts used for this project will be maintained at: <https://github.com/andrew10043/mpog_peds_ponv>.

## Study Population

Using the Intraoperative Research Standard starting population, we will include all patients age 0-17 undergoing anesthesia care at participating institutions from January 1st, 2015 through January 31st, 2023. Complete inclusion and exclusion criteria for our broad cohort are included below:

-   Inclusion criteria
    -   Procedure date from 01/01/2018 - 01/31/2023
    -   Anesthetic encounter $\geq$ 15 minutes
    -   Age at time of procedure $\leq$ 17 years
-   Exclusion criteria
...
While the MPOG PONV-04 metric explicitly excludes patients under the age of three, this is not a shared exclusion with the formal published guidelines, which use age \< 3 as a single risk factor (rather than an exclusion from consideration for prophylaxis). To examine the association of interest and to understand the implications of the guidelines in this younger population, we will use this expanded cohort of all patients $\leq$ 17 years of age in addition to a more limited cohort using all MPOG PONV-04 specified criteria (e.g., age $\leq$ 3 years).

After presentation and discussion at the initial PCRC meeting, we have determined that an a priori four-level model (procedures, patients, clinicians, hospitals) will be our initial approach. If logistical limitations arise (computation time, etc.) we will consider utilizing an index case exclusion approach to remove one level from the model while not inappropriately modeling repeated patient encounters.
...
| [Race](https://phenotypes.mpog.org/Race)                                      | Nominal          | Race of the patient                                                                 |
| Ethnicity                                                                     | Nominal          | Hispanic ethnicity indicator variable (derived from the MPOG "Race" variable above) |
...
| [Medical School Affiliation](https://phenotypes.mpog.org/Medical%20School%20Affiliation)         | Nominal          | Indicates whether the case occurred at an institution affiliated with a medical school                  |
...
| [Hospital Bed Size](https://phenotypes.mpog.org/Hospital%20Bed%20Size)                           | Ordinal          | Categorical value indicating the bed-size for each hospital                                             |
| [Location Tag](https://phenotypes.mpog.org/Location%20Tags)                                      | Nominal          | Used to identify cases occurring at ambulatory centers                                                  |
...
| [Procedure Type: Adenotonsillectomy](https://phenotypes.mpog.org/Procedure%20Type:%20Adenotonsillectomy)                         | Nominal          | Determines whether the case includes Tonsillectomy, Adenoidectomy, Adenotonsillectomy (or control of postop tonsil bleed)               |
...
The *primary provider* phenotype will be used to provide a unique attending ID for multilevel modeling purposes. It will also be used to classify the status of two in-room clinician variables: resident ever present and non-resident ever present (CRNA or AA).
...
| Risk Factor: Opioid                                                       | Nominal          | Indicator variable for patients who are administered long-acting opioids                                         |
...
More recently, *Nafa* and *Heiss*[@RN1230] have outlined an approach aimed at circumventing this issue by using only the location and scale of the propensity score weights in the outcome model and propagating the "uncertainty by placing a prior on the scale component of the weights." Here, they parameterize propensity score weights as:
...
$$
\textit{logit }(p_{ich}) = \beta_0 + \beta_{1}X_{ich} + \beta_{2}K_{ch} + \beta_{3}Z_{h}
$$
...
Taken together, evidence from simulation studies appears to favor a either a fixed effects or random effects approach to specification of the propensity score model, with some variability in simulation studies as noted above. A strictly fixed effects approach in the proposed setting is relatively unattractive given the fully nested nature of the clusters and the large number of clinician-level clusters, many of which may be quite small. One alternative may be a hybrid approach, in which hospital is modeled as a fixed effect, with nested random effects for clinician within hospitals. However, based on input from the MPOG statistical review team, this approach was deemed suboptimal. Thus, we will plan to use a random effects propensity score model, with random intercepts at the patient, clinician, and hospital levels.

Importantly, this group is separately submitting a PCRC proposal for a descriptive analysis of PONV prophylaxis compliance in which the relationships between individual-, clinician-, and hospital-level variables with compliant prophylaxis allocation will be extensively explored. Results from this modeling will be illuminating with respect to the ideal propensity score modeling approach and will help guide our eventual selection.

Variable selection for inclusion in the propensity score model (and outcome model) will be guided by construction of a causal directed acyclic graph (DAG) outlining the proposed relationships between the exposure, outcome, and covariates of interest (@fig-dag). While the DAG below represents a starting point, we will use information gathered from the aforementioned descriptive analysis to understand more about variables associated with PONV prophylaxis compliance and further expand this figure. Prior to propensity score weighting, covariates of interest will be compared across cohorts (compliant vs. non-compliant prophylaxis) using standardized mean differences.

```{r fig-dag, echo=FALSE, dpi=600, fig.height=9, fig.width=20, fig.align='center', cache=TRUE, fig.cap="DAG Showing proposed relationships amongst the exposure, outcome and covariates of interest"}
...
The propensity score model will be a multilevel, Bayesian Bernoulli model with random intercepts at the patient, clinician, and hospital-levels. Further addition of covariate-cluster interactions (either interaction terms between hospital-specific intercept and patient-level covariates, or random slopes for clinicians) will be considered based on initial descriptive analyses.

$$
\begin{aligned}
\textbf{Outcome: }{}        &\text{compliant}_{pich} \sim \textit{Bernoulli }(p_{pich})\\
\textbf{Procedure Level: }{}  &\text{logit}(p_{pich}) = \beta_{0ich} + \beta_{1}X_{pich} + \beta_{2}K_{ich} + \beta_{3}Z_{ch} + \beta_{4}W_{h} + \epsilon_{pich}\\
\textbf{Patient Level: }{}&\beta_{0ich} = \delta_{00ch} + Q_{0ich}\\
\textbf{Clinician Level: }{}&\delta_{00ch} = \theta_{000h} + U_{00ch}\\
\textbf{Hospital Level: }{} &\theta_{000h} = \gamma_{0000} + V_{000h}\\
\end{aligned}
$$ with, $$
\begin{aligned}
Q_{0ich} &\sim \textit{Normal }(0, \sigma_{i}^2)\\
U_{00ch} &\sim \textit{Normal }(0, \sigma_{c}^2)\\
V_{000h} &\sim \textit{Normal }(0, \sigma_{h}^2)\\
\end{aligned}
$$

\noindent where, $\beta_{0ich}$ represents the patient, clinician and hospital specific intercept, composed of a fixed population intercept ($\gamma_{0000}$), and patient, clinician, and hospital-level random effects ($Q_{0ich}, U_{00ch}, V_{000h}$). $\beta_{1} \ldots \beta_{4}$ represent fixed effects coefficients for all included procedure, patient, clinician, and hospital-level covariates, with their associated procedure, patient, clinician and hospital-level covariates ($X_{pich}, K_{ich}, Z_{ch}, W_{h}$). Continuous variables (e.g., age, duration of anesthesia, etc.) will be modeled using thin plate splines.
...
\gamma_{0000}{} &\sim \textit{Normal }(0, 1.5)\\
\sigma_{i}{}   &\sim \textit{Half-Cauchy }(0, 1)\\
\sigma_{c}{}   &\sim \textit{Half-Cauchy }(0, 1)\\
\sigma_{h}{}   &\sim \textit{Half-Cauchy }(0, 1)\\
\end{aligned}
$$
...
Based on assumed causal associations and the prevalence of hospital and clinician-level practice pattern variability, the *a priori* fully-specified primary outcome model will be a multilevel, propensity score weighted, Bayesian Bernoulli model with random intercepts at the patient, clinician, and hospital-levels, and slopes for the coefficient of interest ($\beta_{compliant}$) at the hospital- and clinician-level. Random slopes are included at the hospital and clinician level to account for differential response to adequate prophylaxis amongst similar patients within clinician and hospital clusters that may not be fully captured by measured variables. This full model will be compared to more sparse models (e.g., removal of random slopes) with respect to predicted out of sample performance using the widely applicable information criterion (WAIC). The full model can be represented as:

$$
\begin{aligned}
\textbf{Outcome: }{}        &\text{PONV}_{pich} \sim \textit{Bernoulli }(p_{pich})^{\tilde{w}_{i}}\\
\textbf{Procedure Level: }{}  &\text{logit}(p_{pich}) = \beta_{0ich} + \beta_{1ch}x_{1p} + \beta_{2}x_{2p} + \cdots + \beta_{j}x_{jp} + \epsilon_{pich}\\
\textbf{Patient Level: }{}&\beta_{0ich} = \delta_{00ch} + Q_{0ich}\\
\textbf{Clinician Level: }{}&\delta_{00ch} = \theta_{000h} + U_{00ch}\\
                            &\beta_{1ch} = \delta_{10h} + U_{1ch}\\
\textbf{Hospital Level: }{} &\theta_{000h} = \gamma_{0000} + V_{000h}\\
                            &\delta_{10h} = \eta_{100} + V_{10h}\\
\end{aligned}
$$ \noindent with, $$
\tilde{w}_{i} = \kappa_{i} + \lambda_{i}\cdot\pi(\lambda)
$$

\noindent where, $\beta_{0ich}$ represents the patient, clinician and hospital specific intercept, $\beta_{1ch}$ represents the clinician and hospital specific treatment effect (with $x_{1p}$ = compliance status), and $\beta_{2} \ldots \beta_{j}$ represent additional fixed effects coefficients. $Q_{0ich}, U_{00ch}$ and $V_{000h}$ represent clinician-, and hospital-level random intercepts, respectively, while $U_{1ch}$ and $V_{10h}$ represent clinician- and hospital-level random slopes for the treatment effect. Finally, $\gamma_{0000}$ is the fixed intercept and $\eta_{100}$ is the fixed effect estimate for the treatment and $\tilde{w}_{i}$ is the propensity score weight term. The distribution and correlation structure between random intercepts and slopes at both the hospital and clinician level are described as:
...
\gamma_{0000}{} &\sim \textit{Normal }(-1.5, 0.75)\\
\eta_{100}{} &\sim \textit{Normal }(0, 1)\\
\sigma_{i}{} &\sim \textit{Half-Cauchy }(0, 1)\\
...
```{r fig-intercept, echo=FALSE, dpi=600, fig.height=3, fig.width=3, fig.align='center', cache=FALSE, fig.cap="Prior distribution for the fixed intercept on the probability scale."}
...
or_prior_plot <-
  ...
  stat_halfeye(aes(x = or),
               fill = degas[2]) +
  scale_x_log10(breaks = c(0.03, 0.1, 0.3, 1, 3, 10, 30),
                labels = c("0.03", "0.10", "0.30", "1.0", "3.0", "10", "30")) + 
...
We will explore the impact of prophylaxis compliance on PONV in a number of sub-populations of interest. These include pediatric patients undergoing radiology procedures (specifically MRI), and those undergoing endoscopy. These each represent extremely common procedures in the practice of pediatric anesthesiology and may offer unique insights into the relationship between prophylaxis and PONV given their overall procedural homogeneity.
...
\phi_{00}{}    & = 0.03\\
\phi_{00}{}    & = 0.03\\
R_c = \begin{pmatrix} 1 & -0.2 \\ -0.2 & 1 \end{pmatrix} \ {}& \ 
R_h = \begin{pmatrix} 1 & -0.2 \\ -0.2 & 1 \end{pmatrix}\\
\end{aligned}
$$

\noindent for $i = 1, \ldots, n_{ch}$, $c = 1, \ldots, C_h$, and $h = 1, \ldots, H$. We then iteratively varied the sample size ($N$ = 1000, 10,000, 100,000), and for each value of $N$ fit 50 Bayesian outcome models with the structure and priors as outlined in the outcome modeling section. The basic results of the stimulation are shown in the plots below. @fig-power-facet shows posterior distributions of the odds ratio for the fixed effect estimates ($\gamma_{100}$) accompanied by their 95% credible intervals.

```{r fig-power-facet, echo=FALSE, dpi=600, fig.height=6, fig.width=9, fig.align='center', cache=TRUE, fig.cap="Precision simulation results. Each facet represents a different total sample size (N). For each facet (N), 50 simulations were performed, and each is represented by the posterior median odds ratio (point) and 95% credible intervals (lines), ordered by median OR. The red line marks OR = 1 (no effect) while the blue line marks OR = 0.9 (the true simulated effect). The N = 100,000 facet contains an inset to more easily display the precision of these estimates around the true simulated value."}
...
power_sims <- readRDS("~/Desktop/projects/mpog_multi_ponv/data/power_sims.rds")

labs <- c("N = 1,000", "N = 10,000", "N = 100,000")
names(labs) <- c("1k", "10k", "100k")

main_plot <- 
  ggplot(data = power_sims, aes(x = reorder_within(seed, exp(Estimate), n), 
                                y = exp(Estimate), ymin = exp(Q2.5), 
                                ymax = exp(Q97.5))) +
    geom_hline(yintercept = 0.9, color = renoir[4]) +
    geom_hline(yintercept = 1, color = renoir[6]) +
    geom_pointrange(fatten = 1/2) + 
    scale_x_reordered("Simulations (n = 50)", breaks = NULL) +
    coord_cartesian(ylim = c(0, 2.5)) + 
    facet_wrap(~ n, scales = "free_x",
               labeller = labeller(n = labs)) + 
    ylab(expression(exp(gamma[100]))) + 
...
inset_plot <- 
  ggplot(data = power_sims %>% filter(n == "100k"), 
         aes(x = reorder(seed, exp(Estimate)),
...
main_plot +
  annotation_custom2(grob = ggplotGrob(inset_plot), data = power_sims %>% filter(n == "100k"), ymin = 1.5, ymax = 2.45, xmin = 0, xmax = 50)
```

In these simulations we find that the precision in our effect estimate of interest rapidly increases as sample size moves from 1,000 to 100,000 at which point all 95% credible intervals no longer include OR = 1. As an alternative visual representation of the precision of these estimates we can examine the distribution of their widths (on the OR scale). This is shown in @fig-power-width below.

```{r fig-power-width, echo=FALSE, dpi=600, fig.height=4, fig.width=9, fig.align='center', cache=TRUE, fig.cap="Precision simulation results. Again, each facet represents a different total sample size (N). Here we show the distribution of the widths of the 95% credible interval for the OR, an alternative metric for assessing the precision of our estimates. Facets for N = 10,000 and N = 100,000 contain insets to more clearly show the distribution of widths for these cases."}

main_plot <- 
  ggplot(data = power_sims,
         aes(x = exp(Q97.5) - exp(Q2.5))) +
    geom_histogram(fill = renoir[12], color = "white") + 
    facet_wrap(~ n, scales = "fixed",
               labeller = labeller(n = labs)) + 
    labs(y = "Count",
         x = "Width of 95% Credible Interval for OR") + 
    theme(panel.spacing = unit(2, "lines"),
...
inset_plot_100 <-
  ggplot(data = power_sims %>% filter(n == "100k")) + 
  geom_histogram(bins = 50, aes(x = exp(Q97.5) - exp(Q2.5)), 
                 fill = renoir[12], color = "white") + 
  scale_y_continuous(limits = c(0, 15), breaks = seq(0, 15, by = 5)) +
  scale_x_continuous(limits = c(0, 0.11), breaks = c(0, 0.025, 0.05, 0.075, 0.1),
                     labels = c(0, "", 0.05, "", 0.01)) + 
  labs(x = NULL, y = NULL) + 
  theme(plot.background = element_rect(color = "black"),
        axis.text = element_text(size = 12))

inset_plot_10 <-
  ggplot(data = power_sims %>% filter(n == "10k")) + 
  geom_histogram(bins = 50, aes(x = exp(Q97.5) - exp(Q2.5)), 
                 fill = renoir[12], color = "white") + 
...
main_plot +
  annotation_custom2(grob = ggplotGrob(inset_plot_100), data = power_sims %>% filter(n == "100k"), ymin = 1, ymax = 20, xmin = 0.45, xmax = 1.45) + 
  annotation_custom2(grob = ggplotGrob(inset_plot_10), data = power_sims %>% filter(n == "10k"), ymin = 1, ymax = 20, xmin = 0.45, xmax = 1.45)
```

Examining the inset of the N = 100,000 simulation, we see that all 95% credible intervals for the OR of interest were \< 0.1 in width, representing very precise estimates. Given the excellent results obtained from the N = 100,000 simulation, further increases in sample size were not pursued. For reference, each 100,000 sample model in this simulation took approximately \~23 minutes to run.
...
One particular area of importance with regard to missing data for this analysis is the presence of missing PONV risk factor data. While age, sex, procedure type, opioid utilization and anesthetic duration should all theoretically be routinely collected (and thus broadly available) in the electronic medical record (and MPOG database), we are aware that documentation of PONV history may be much less consistent. Specifically, unless this is a required data element during documentation of an anesthetic encounter, missingness of this variable likely follows a missing not at random (MNAR) pattern, with reduced likelihood of missing data for patients who *do* have a history of PONV compared to those who *do not*. For this reason, we plan to carefully evaluate the patterns of missingness of PONV history documentation across centers in our descriptive study to further understand how best to approach centers with poor or inconsistent documentation of this important exposure variable.
