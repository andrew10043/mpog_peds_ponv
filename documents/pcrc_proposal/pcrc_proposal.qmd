...
Variable selection for inclusion in the propensity score model (and outcome model) will be guided by construction of a causal directed acyclic graph (DAG) outlining the proposed relationships between the exposure, outcome, and covariates of interest (@fig-dag). Prior to propensity score weighting, covariates of interest will be compared across cohorts (compliant vs. non-compliant prophylaxis) using standardize mean differences. 
...
\textbf{Patient Level: }{}  &\text{logit}^{-1}(p_{ich}) = \beta_{0ch} + \beta_{1ch}w_{ich} + \beta_{2}X_{i} + \cdots + \beta_{j}X_{i}\\
...
where, $\beta_{0ch}$ represents the clinician and hospital specific intercept, $\beta_{1ch}$ represents the clinician and hospital specific treatment effect, and $\beta_{2} \ldots \beta_{j}$ represent additional fixed effects coefficients. $U_{0ch}$ and $V_{00h}$ represent clinician-, and hospital-level random intercepts, respectively, while $U_{1ch}$ and $V_{10h}$ represent clinician- and hospital-level random slopes for the treatment effect. Finally, $\gamma_{000}$ is the fixed intercept and $\gamma_{100}$ is the fixed effect estimate for the treatment. The distribution and correlation structure between random intercepts and slopes at both the hospital and clinician level are described as:
...
Finally, priors for all parameters are outlined to complete the model:

$$
\begin{aligned}
\beta_{j}{}    &\sim \textit{Normal }(0, 1)\\
\gamma_{000}{} &\sim \textit{Normal }(-2, 1)\\
...
The full set of prior distributions is delineated in the prior section. Selection of prior distributions is based primarily on baseline knowledge of the incidence of PONV in the pediatric population (particularly from prior single center work) and the use of prior predictive simulation. For the fixed intercept we will use $\textit{Normal}(-1.5, 0.75)$, which implies the distribution of PONV probabilities shown in @fig-intercept for the average patient receiving non-compliant prophylaxis, with covariates at the mean or in the reference class.

```{r fig-intercept, echo=FALSE, dpi=600, fig.height=3, fig.width=3, fig.align='center', cache=TRUE, fig.cap="Prior distribution for the intercept on the probability scale."}
...
For the coefficient of interest ($\beta_{compliant}$) our primary prior will be relatively conservative and centered on no effect: $\textit{Normal(0, 1)}$. Simulated distributions using this prior are shown below for (A) $\beta_{compliant}$ on the log odds scale, (B) $\text{OR}_{compliant}$, and (C) the conditional absolute risk reduction ($\text{ARR}_{compliant}$) for patients with all covariates at the reference class.
...
beta_prior_plot <-
  ...
  stat_halfeye(aes(x = beta),
               fill = degas[5]) +
  labs(x = expression(bold(beta["compliant"])),
       y = "Density") + 
...
or_prior_plot <-
  ...
  stat_halfeye(aes(x = or),
               fill = degas[2]) +
  labs(x = expression(bold(OR["compliant"])),
       y = "Density") + 
...
arr_prior_plot <- 
  ...
  stat_halfeye(aes(x = arr),
               fill = degas[4]) + 
  labs(x = expression(bold(ARR["compliant"])),
       y = "Density") + 
...
```{r fig-random-prior, echo=FALSE, dpi=600, fig.height=3, fig.width=5, fig.align='center', cache=TRUE, fig.cap="Prior distributions for random effects parameters."}
